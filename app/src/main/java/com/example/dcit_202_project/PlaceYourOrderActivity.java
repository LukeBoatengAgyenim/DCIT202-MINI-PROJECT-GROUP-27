...
import com.example.dcit_202_project.adapters.PlaceYourOrderAdapter;
...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    private void calculateTotalAmount(RestaurantModel restaurantModel) {
        ...
        tvSubtotalAmount.setText("GH₵"+String.format("%.2f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("GH₵"+String.format("%.2f", restaurantModel.getDelivery_charge()));
            subTotalAmount += restaurantModel.getDelivery_charge();
        }
        tvTotalAmount.setText("GH₵"+String.format("%.2f", subTotalAmount));
    }
